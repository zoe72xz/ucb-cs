...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        legalActions = self.getLegalActions(state)
        if not legalActions:
            return None  # No action if no legal moves
        ...
        if max_q_value <= 0:  
            unseen_actions = [action for action in legalActions if q_values[action] == 0]
            if unseen_actions:
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        newQ = (1 - self.alpha) * currentQ + self.alpha * (reward + self.discount * nextQ)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward: float):
        ...
        nextQValue = max(self.getQValue(nextState, a) for a in self.getLegalActions(nextState)) if self.getLegalActions(nextState) else 0.0
