...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    cornersRemaining = [corners[i] for i, visited in enumerate(state[1]) if not visited]

    if not cornersRemaining:
        return 0

    if len(cornersRemaining) == 1:
        return manhattanDistance(currentPosition, cornersRemaining[0])

    points = [currentPosition] + cornersRemaining
    distances_dict = {}
    for p1, p2 in combinations(points, 2):
        distances_dict[(p1, p2)] = manhattanDistance(p1, p2)
        distances_dict[(p2, p1)] = distances_dict[(p1, p2)]

    mst_cost = 0
    visited = {points[0]}
    edges = sorted((distances_dict[(points[0], p)], p) for p in points[1:])

    while edges:
        cost, next_point = edges.pop(0)
        if next_point not in visited:
            mst_cost += cost
            visited.add(next_point)
            edges.extend((distances_dict[(next_point, p)], p) for p in points if p not in visited)
            edges.sort()
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    distances = {}
    for p1, p2 in combinations(points, 2):
        distances[(p1, p2)] = manhattanDistance(p1, p2)
        distances[(p2, p1)] = distances[(p1, p2)]

    mst_cost = 0
    visited = {points[0]}
    edges = sorted((distances[(points[0], p)], p) for p in points[1:])
    ...
    while edges:
        ...
        if next_point not in visited:
            mst_cost += cost
            visited.add(next_point)
            edges.extend((distances[(next_point, p)], p) for p in points if p not in visited)
            edges.sort()
